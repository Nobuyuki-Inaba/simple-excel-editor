...
import { ExcelDocument, SheetData } from './ExcelDocument';
...
export class ExcelEditorProvider implements vscode.CustomEditorProvider<ExcelDocument> {
      ...
      this.pendingData.set(key, async ({ sheetName, columns, rows }) => {
        ...
        try {
          for (const [name] of document.cache) {
            await this.persistSheet(document, name, targetExcelPath);
          }
          resolve();
        } catch (err: unknown) {
          reject(err);
  ...
  private async persistSheet(
    ...
    await this.runner.csvToExcel(csvPath, targetExcelPath, sheetName);
  }
